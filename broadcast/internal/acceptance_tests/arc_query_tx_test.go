...
func TestQueryTransaction(t *testing.T) {
	t.Run("Should successfully query from multiple ArcClients", func(t *testing.T) {
		...
		arc1 := arc3.NewArcClient(&broadcast.ArcClientConfig{APIUrl: "http://arc1-api-url", Token: "arc1-token"}, httpClientMock)
		arc2 := arc3.NewArcClient(&broadcast.ArcClientConfig{APIUrl: "http://arc2-api-url", Token: "arc2-token"}, httpClientMock)
	...
	t.Run("Should return error if all ArcClients return errors", func(t *testing.T) {
		...
		assert.Error(t, err)
		assert.Nil(t, result)
		assert.EqualError(t, err, broadcast_api.ErrAllBroadcastersFailed.Error())
	})
